...
When working in retail, you might need to access sales orders created on the Sales app from the POS
register to finalize the sale.
...
Select a quotation or sales order, and on the popup that opens, choose the desired settlement method.
...
<../../inventory_and_mrp/barcode/setup/barcodelookup>`.

.. note::
   Employees can :ref:`log into the POS by scanning a badge <pos/employee_login/badge>`. To set
   this up:

   #. Open the **Employees** app and select an employee.
   #. In the :guilabel:`HR Settings` tab, fill in the :guilabel:`PIN Code` or :guilabel:`Badge ID`.
...
rather than a new product. These are commonly used for items nearing their expiration date.

.. example::
   A carton of milk is expiring tomorrow. By scanning a barcode following the nomenclature for a
   50% discount (e.g., a barcode starting with `2250`), Odoo automatically applies a 50% price
   reduction to the milk in the current transaction.

Barcode nomenclature
--------------------

Barcode nomenclatures define how barcodes are recognized and categorized. When a barcode is scanned,
it is associated with the first rule that has a matching pattern. The pattern syntax follows
standardized prefixing; a barcode is matched if the pattern matches the barcode's prefix.
...
.. seealso::
   - :doc:`../../inventory_and_mrp/barcode/operations/barcode_nomenclature`
