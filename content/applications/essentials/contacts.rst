...
- :guilabel:`Partner Level`: A drop-down menu where you can select the designated
  :ref:`partner <contacts/partner-assignment>` level for this contact.
...
The :guilabel:`Sales & Purchase` tab only appears when the *Sales*, *Purchase*, or *Point of Sale*
applications are installed. Each of these apps will add another section to this tab when they are
installed.
...
Under the :guilabel:`Purchase` heading, select how :abbr:`Group RFQs (requests for quotation)` for
...
The :guilabel:`Internal Notes` tab is a space where notes for other users can be left on the contact
form. There are no settings that can be configured within this tab.
...
.. example::
   ...
   quotations related to this customer from the *Sales* app are accessible.
...
.. tip::
   ...
   click the :icon:`fa-cog` :guilabel:`Actions` menu again at the top of the archived contact form,
